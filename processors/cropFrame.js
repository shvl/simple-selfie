...
function cropFrame(originalFrame, frame, data) {
    return __awaiter(this, void 0, void 0, function () {
        var result, width, height, x, y, iy, ix, originalIndex, index;
        return __generator(this, function (_a) {
            result = new Uint8ClampedArray(frame.width * frame.height * 4);
            width = frame.width, height = frame.height, x = frame.x, y = frame.y;
            for (iy = 0; iy < height; iy++) {
                for (ix = 0; ix < width; ix++) {
                    originalIndex = (iy + y) * originalFrame.width + (ix + x);
                    index = iy * width + ix;
